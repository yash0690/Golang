...
func cookies(res http.ResponseWriter, req *http.Request){
	...
	mycookie, err := req.Cookie("secure-session-id")
	if(err != nil){
...
func main(){
	...
	go http.ListenAndServe("localhost:8080", http.RedirectHandler("https://localhost:10443/", http.StatusMovedPermanently))
	http.HandleFunc("/",cookies)


	http.ListenAndServeTLS("localhost:10443","cert.pem","key.pem",nil)
}
